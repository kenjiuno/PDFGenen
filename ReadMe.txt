========================================================================
ATL (ACTIVE TEMPLATE LIBRARY) : PDFGenen プロジェクトの概要
========================================================================

この PDFGenen プロジェクトは、ユーザーがダイナミック リンク ライ
...
PDFGenen.idl
    ...
    このファイルは MIDL コンパイラによって処理され、次のものが生成されます。
        C++ のインターフェイス定義と GUID 宣言 (PDFGenen.h)
        GUID 定義                              (PDFGenen_i.c)
        タイプ ライブラリ                      (PDFGenen.tlb)
        マーシャリング コード                  (PDFGenen_p.c および dlldata.c)

PDFGenen.h
    このファイルには、C++ のインターフェイス定義および 
    PDFGenen.idl. で定義される項目の GUID 宣言が含まれます。
...
StdAfx.h, StdAfx.cpp
    ...
    PDFGenen.pch とプリコンパイル済み型ファイル StdAfx.obj 
    をビルドするために使用します。
...
PDFGenenps.vcproj
	...
	PDFGenen_i.c と PDFGenen_p.c が生成され
        ます。
